...
1. 기존 파일 개수와 경로(`기존 파일 N개:`)를 출력합니다.
...
4. 이미 이 저장소의 원본을 가리키는 링크는 `이미 연결됨`으로 통과합니다.

스킬 설치 시에는 이 저장소에서 삭제된 스킬을 가리키는 끊어진 링크를 자동으로 제거합니다(`삭제된 스킬 링크 제거: ...`). 저장소 밖을 가리키는 링크는 건드리지 않습니다.
