...
name: diagram-design
description: >
  Create branded editorial architecture, flowchart, sequence, ERD, and 39 visual types of diagrams as standalone HTML/SVG/PNG.
  사용자가 /diagram-design 을 직접 입력하거나 'diagram-design' 스킬을 명시적으로 지목했을 때만 작동한다.
  일반적인 설명, 아키텍처, 다이어그램 요청이라도 스킬을 명시하지 않았다면 이 스킬을 쓰지 않고 평소대로 답한다.
  eli5 스킬 내부에서 고품질 에디토리얼 SVG 다이어그램을 생성하기 위한 디자인 레퍼런스로도 활용된다.
license: MIT
metadata:
  version: "2.6"
---
